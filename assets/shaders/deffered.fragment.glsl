...
void main() {
    vec4 color = vec4(0f,0f,0f,0f);
		...
		#if defined(ambientFlag) && defined(separateAmbientFlag)
			#ifdef shadowMapFlag
				color.rgb = (diffuse.rgb * (v_ambientLight + getShadow() * v_lightDiffuse));
				//color.rgb = texture2D(u_shadowTexture, v_shadowMapUv.xy);
			#else
				color.rgb = (diffuse.rgb * (v_ambientLight + v_lightDiffuse));
			#endif //shadowMapFlag
		#else
			#ifdef shadowMapFlag
				color.rgb = getShadow() * (diffuse.rgb * v_lightDiffuse);
			#else
				color.rgb = (diffuse.rgb * v_lightDiffuse);
			#endif //shadowMapFlag
		#endif
	#else
		#if defined(specularTextureFlag) && defined(specularColorFlag)
		...
		#if defined(ambientFlag) && defined(separateAmbientFlag)
			#ifdef shadowMapFlag
			color.rgb = (diffuse.rgb * (getShadow() * v_lightDiffuse + v_ambientLight)) + specular;
				//color.rgb = texture2D(u_shadowTexture, v_shadowMapUv.xy);
			#else
				color.rgb = (diffuse.rgb * (v_lightDiffuse + v_ambientLight)) + specular;
			#endif //shadowMapFlag
		#else
			#ifdef shadowMapFlag
				color.rgb = getShadow() * ((diffuse.rgb * v_lightDiffuse) + specular);
			#else
				color.rgb = (diffuse.rgb * v_lightDiffuse) + specular;
			#endif //shadowMapFlag
		#endif
	#endif //lightingFlag
