package de.macbury.hashbot.core.graphics.camera;
...
public class RTSCameraController extends InputAdapter {
  ...
  private boolean changePressStateFor(int keycode, boolean state) {
    ...
    if (HashBot.input.isEqual(InputManager.Action.CameraRotateLeft, keycode)) {
      rotateLeftPressed = state;
      acted = true;
    }

    if (HashBot.input.isEqual(InputManager.Action.CameraRotateRight, keycode)) {
      rotateRightPressed = state;
      acted = true;
    }

    if (HashBot.input.isEqual(InputManager.Action.CameraForward, keycode)) {
      forwardPressed = state;
      acted = true;
    }

    if (HashBot.input.isEqual(InputManager.Action.CameraBackward, keycode)) {
      backwardPressed = state;
      acted = true;
    }

    if (HashBot.input.isEqual(InputManager.Action.CameraLeft, keycode)) {
      leftPressed = state;
      acted = true;
    }

    if (HashBot.input.isEqual(InputManager.Action.CameraRight, keycode)) {
      rightPressed = state;
      acted = true;
    }

    if (HashBot.input.isEqual(InputManager.Action.CameraTiltBackward, keycode)) {
      tiltBackward = state;
      acted = true;
    }

    if (HashBot.input.isEqual(InputManager.Action.CameraTiltForward, keycode)) {
      tiltForward = state;
      acted = true;
    }
